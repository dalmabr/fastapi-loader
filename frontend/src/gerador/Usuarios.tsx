...
export default function Usuarios() {
  ...
  async function atualizarRole(userId: string, novoRole: 'admin' | 'operator') {
    try {
      const { error } = await supabase
        ...
        .eq('id', userId);
      if (error) throw error;
      setProfiles(prev => prev.map(p => p.id === userId ? { ...p, role: novoRole } : p));
      setEditandoId(null);
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-[#3C2E26] text-white h-10">
                <th className="text-left px-4 text-xs font-semibold uppercase tracking-wider w-[40%]">E-mail</th>
                <th className="text-left px-4 text-xs font-semibold uppercase tracking-wider w-[20%]">Perfil</th>
                <th className="text-left px-4 text-xs font-semibold uppercase tracking-wider w-[20%]">Status</th>
            ...
            <tbody>
              {carregando ? (
                ...
                <tr>
                  <td colSpan={4} className="py-8 text-center text-[#999999]">Nenhum usuário encontrado</td>
                </tr>
              ) : (
                profiles.map(p => (
                  <tr key={p.id} className="border-b border-[#EDE8E2] hover:bg-[#FDF8F3] h-10">
                    <td className="px-4 text-[#3C2E26] font-medium">{p.email}</td>
                    <td className="px-4">
                      {editandoId === p.id ? (
                        <select
                          value={p.role}
                          onChange={e => atualizarRole(p.id, e.target.value as 'admin' | 'operator')}
                          className="h-8 px-2 text-sm text-[#3C2E26] bg-white border border-[#C4A484] rounded outline-none appearance-none cursor-pointer"
                    ...
                    <td className="px-4 flex items-center justify-center gap-1.5">
                      <button
                        ...
                        title={editandoId === p.id ? 'Fechar' : 'Editar'}
                      >
                        {editandoId === p.id ? (
